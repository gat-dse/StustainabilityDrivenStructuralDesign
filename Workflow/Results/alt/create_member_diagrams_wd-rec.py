...
Diagramme:
1. Durchbiegung w_app vs. zulässige Durchbiegung w_app_adm
...
def lade_daten(excel_file, sheet_name):
    ...
    df["w_app_groesser_als_adm"] = df["w_app"] > df["w_app_adm"]
...
DIAGRAMME = [
    {
        "suffix": "w_app_vs_w_app_adm",
        "spalten": ["w_app_adm", "w_app"],
        "labels": ["w_app_adm ", "w_app"],
        "ylabel": "Durchbiegung w [m]",
        "bedingung": "w_app_groesser_als_adm",
        "bedingung_label": "w_app > w_app_adm",
        "titel": "Durchbiegung w_app vs. w_app_adm",
    },
    {
...
def main():
    if len(sys.argv) >= 2:
        ...
        excel_file = input("Name/Pfad der Excel-Datei: ").strip().strip('"')
        if not excel_file:
            print("Keine Excel-Datei angegeben. Abbruch.")
            sys.exit(1)
        sheet_name = DEFAULT_SHEET_NAME

    output_dir = os.path.join(os.path.dirname(os.path.abspath(excel_file)), "DataAnalysis_plots")
