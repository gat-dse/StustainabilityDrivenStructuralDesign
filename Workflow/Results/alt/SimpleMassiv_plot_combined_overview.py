"""
Kombiniert alle fuenf Einzeldiagramme (w_use/w_use_ger, fwger(phi=2)_calc, roh/rohs, h/d_calc, co2)
fuer Members_rc_rec_simple_massiv in einer Uebersichtsgrafik:
5 Zeilen (je eine Kenngroesse) x 10 Spalten (je eine Spannweite von 3 bis 12 m).
...
df = pd.read_sql(
    f'SELECT Member_ID, l_tot, w_use, w_use_ger, "fwger(phi=2)_calc", roh, rohs, "h/d_calc", co2 '
    f'FROM {table_name}',
    con)
con.close()

value_cols = ["l_tot", "w_use", "w_use_ger", "fwger(phi=2)_calc", "roh", "rohs", "h/d_calc", "co2"]
...
zeilen = [
    {"cols": ["w_use", "w_use_ger"], "colors": [farbe_1, farbe_2],
     ...
     "ylabel": "Durchbiegung w [m]", "grenzwerte": []},
    {"cols": ["fwger(phi=2)_calc"], "colors": [farbe_1],
     "labels": ["fwger(phi=2)_calc"],
     "ylabel": "fwger (phi=2) [-]", "grenzwerte": [3.0]},
    {"cols": ["roh", "rohs"], "colors": [farbe_1, farbe_2],
     "labels": ["roh (unten)", "rohs (oben)"],
     "ylabel": "Bewehrungsgehalt [-]", "grenzwerte": [0.0015, 0.02]},
    {"cols": ["h/d_calc"], "colors": [farbe_1],
     "labels": ["h/d_calc"],
     "ylabel": "h/d [-]", "grenzwerte": []},
    {"cols": ["co2"], "colors": [farbe_1],
     "labels": ["co2"],
     "ylabel": "co2 [kgCO2eq/m²]", "grenzwerte": []},
]

# ==============================================================================
# PLOT: 5 ZEILEN (KENNGROESSEN) x 10 SPALTEN (SPANNWEITEN)
# ==============================================================================
...
for row_idx, zeile in enumerate(zeilen):
    for col_idx, l in enumerate(spannweiten):
        ...
        for grenzwert in zeile["grenzwerte"]:
            ax.axhline(y=grenzwert, color=farbe_grenzwert, linestyle="--", linewidth=1.2,
...
fig.suptitle("Uebersicht: Durchbiegung, fwger(phi=2), Bewehrungsgehalt, h/d und co2 je Member, "
             "kategorisiert nach Spannweite\n(Members_rc_rec_simple_massiv)", fontsize=16)
fig.tight_layout(rect=[0, 0.02, 1, 0.96])

fig.savefig("SimpleMassiv_combined_overview.pdf", bbox_inches="tight")
fig.savefig("SimpleMassiv_combined_overview.png", dpi=300, bbox_inches="tight")
print("Diagramm gespeichert: SimpleMassiv_combined_overview.pdf / .png")
