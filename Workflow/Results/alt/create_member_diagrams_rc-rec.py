...
Diagramme:
1. Durchbiegung w_use (ungerissen) vs. w_use_ger (gerissen)
2. Biegewiderstand mu_max(MRd) vs. Bemessungsmoment mEd_p
3. Querkraftwiderstand vu_PB_p(VRd) vs. Bemessungsquerkraft vEd
...
FARBE_1 = "#0072B2"           # Widerstand / ungerissen
...
DIAGRAMME = [
    {
        "suffix": "w_use_vs_w_use_ger",
        "spalten": ["w_use", "w_use_ger"],
        "labels": ["w_use (ungerissen)", "w_use_ger (gerissen)"],
        "ylabel": "Durchbiegung w [m]",
        "bedingung": "ger_kleiner_als_use",
        "bedingung_label": "w_use_ger < w_use",
        "titel": "Durchbiegung w_use vs. w_use_ger",
    },
    {
        "suffix": "mu_max_vs_mEd",
        "spalten": ["mu_max(MRd)", "mEd_p"],
        "labels": ["mu_max (MRd)", "mEd_p"],
        "ylabel": "Moment [kNm]",
        "bedingung": "med_groesser_als_mrd",
        "bedingung_label": "mEd_p > mu_max(MRd)",
        "titel": "Biegewiderstand mu_max(MRd) vs. Bemessungsmoment mEd_p",
    },
    {
        "suffix": "vEd_vs_vu_PB_p",
        "spalten": ["vu_PB_p(VRd)", "vEd"],
        "labels": ["vu_PB_p (VRd)", "vEd"],
        "ylabel": "Querkraft [kN]",
        "bedingung": "ved_groesser_als_vrd",
        "bedingung_label": "vEd > vu_PB_p(VRd)",
        "titel": "Querkraftwiderstand vu_PB_p(VRd) vs. Bemessungsquerkraft vEd",
    },
]
...
def plot_einzeldiagramm(df, spannweiten, diagramm_cfg, dataset_name, output_prefix):
    ...
    for ax, l in zip(axes, spannweiten):
        ...
        for spalte, label, farbe in zip(diagramm_cfg["spalten"], diagramm_cfg["labels"], [FARBE_1, FARBE_2]):
            ax.plot(df_l["member_index"], df_l[spalte], marker="o", markersize=6,
                    linewidth=2, color=farbe, label=label)
...
def plot_kombiniert(df, spannweiten, dataset_name, output_prefix):
    ...
    for row_idx, diagramm_cfg in enumerate(DIAGRAMME):
        for col_idx, l in enumerate(spannweiten):
            ...
            shade_condition_regions(ax, df_l["member_index"], df_l[diagramm_cfg["bedingung"]], FARBE_HINTERLEGUNG)
            for spalte, label, farbe in zip(diagramm_cfg["spalten"], diagramm_cfg["labels"], [FARBE_1, FARBE_2]):
                ax.plot(df_l["member_index"], df_l[spalte], marker="o", markersize=4,
                        linewidth=1.6, color=farbe, label=label)
...
def main():
    if len(sys.argv) < 2:
        print("Verwendung: py create_member_diagrams.py <pfad_zur_excel_datei> [sheet_name]")
        sys.exit(1)
