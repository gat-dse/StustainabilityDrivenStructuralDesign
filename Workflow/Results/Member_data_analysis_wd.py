"""
Erstellt 5 Nachweis-Diagramme (einzeln und als kombinierte Uebersicht) aus einer
rohen "Members_Comparison"-Excel-Datei (Spalte 'key' + 'level', je eine Spalte pro Member) -
demselben Rohformat, wie es struct_analysis.py fuer Holz-Querschnitte exportiert.

Diagramme:
1. Biegewiderstand mu_max (MRd) vs. Bemessungsmoment mEd_p (med)
2. Querkraftwiderstand vu_p (VRd) vs. Bemessungsquerkraft vEd
3. Durchbiegung w_install_adm (zulässig) vs. w_install
4. Durchbiegung w_use_adm (zulässig) vs. w_use
5. Durchbiegung w_app_adm (zulässig) vs. w_app
...
ein Pfad als erstes Argument uebergeben werden - dieser hat dann Vorrang vor EXCEL_DATEI:
    py Member_data_analysis_wd.py [<pfad_zur_excel_datei>] [sheet_name]
...
EXCEL_DATEI = "Members_wd_rec_simple_260902.xlsx"
...
FARBE_2 = "#D55E00"           # Bemessungswert / vorhandene Durchbiegung
...
def lade_daten(excel_file, sheet_name):
    ...
    # Einheiten: Nm -> kNm, N -> kN, m -> mm fuer bessere Lesbarkeit
    df["mu_max"] = df["mu_max"] / 1000
    df["mEd_p"] = df["mEd_p"] / 1000
    df["vu_p"] = df["vu_p"] / 1000
    df["vEd"] = df["vEd"] / 1000
    df["w_install_adm"] = df["w_install_adm"]* 1000
    df["w_install"] = df["w_install"] * 1000
    df["w_use_adm"] = df["w_use_adm"] * 1000
    df["w_use"] = df["w_use"] * 1000
    df["w_app_adm"] = df["w_app_adm"] * 1000
    df["w_app"] = df["w_app"] * 1000
    ...
    df["w_install_groesser_als_w_install_adm"] = df["w_install"] > df["w_install_adm"]
    df["w_use_groesser_als_w_use_adm"] = df["w_use"] > df["w_use_adm"]
    df["w_app_groesser_als_w_app_adm"] = df["w_app"] > df["w_app_adm"]
...
DIAGRAMME = [
    {
        ...
        "labels": ["mu_max (MRd)", "mEd_p (med)"],
        "ylabel": "Moment [kNm]",
        "bedingung": "med_groesser_als_mrd",
        "bedingung_label": "mEd_p > mu_max",
        "titel": "Biegewiderstand mu_max (MRd) vs. Bemessungsmoment mEd_p (med)",
    },
    {
        "suffix": "vEd_vs_vu_p",
        "spalten": ["vu_p", "vEd"],
    ...
    {
        "suffix": "w_install_adm_vs_w_install",
        "spalten": ["w_install_adm", "w_install"],
        "labels": ["w_install_adm (zulässig)", "w_install"],
        "ylabel": "Durchbiegung (Einbauten) w [mm]",
        "bedingung": "w_install_groesser_als_w_install_adm",
        "bedingung_label": "w_install > w_install_adm",
        "titel": "Durchbiegung w_install_adm (zulässig) vs. w_install",
    },
    {
        "suffix": "w_use_adm_vs_w_use",
        "spalten": ["w_use_adm", "w_use"],
        "labels": ["w_use_adm (zulässig)", "w_use"],
        "ylabel": "Durchbiegung (Nutzung) w [mm]",
        "bedingung": "w_use_groesser_als_w_use_adm",
        "bedingung_label": "w_use > w_use_adm",
        "titel": "Durchbiegung w_use_adm (zulässig) vs. w_use",
    },
    {
        "suffix": "w_app_adm_vs_w_app",
        "spalten": ["w_app_adm", "w_app"],
        "labels": ["w_app_adm (zulässig)", "w_app"],
        "ylabel": "Durchbiegung (Aussehen) w [mm]",
        "bedingung": "w_app_groesser_als_w_app_adm",
        "bedingung_label": "w_app > w_app_adm",
        "titel": "Durchbiegung w_app_adm (zulässig) vs. w_app",
    },
]
...
def plot_kombiniert_relativ(df, spannweiten, dataset_name, output_prefix):
    ...
    for row_idx, diagramm_cfg in enumerate(DIAGRAMME):
        ...
        for col_idx, l in enumerate(spannweiten):
            ...
            shade_condition_regions(ax, df_l["member_index"], ausnutzung > 100, FARBE_HINTERLEGUNG)
