"""
Erstellt 8 Nachweis-Diagramme (einzeln und als kombinierte Uebersicht) aus einer
rohen "Members_Comparison"-Excel-Datei (Spalte 'key' + 'level', je eine Spalte pro Member) -
demselben Rohformat, wie es struct_analysis.py fuer Beton-Rippenquerschnitte (RibbedConcrete,
section_type "rc_rib") exportiert.

Im Unterschied zum Rechteck-Querschnitt (Member_data_analysis_rc.py) werden Querkraft und
Rissmoment nicht anhand der Platte (Querrichtung), sondern anhand des Plattenbalkens (PB,
Laengsrichtung der Rippe) gefuehrt, da fuer den Rippenbalken als Bauteil (Member1D) die
Beanspruchung mEd_p/mEd_n/vEd in Laengsrichtung wirkt:
- vu_PB_p (Plattenbalken-Querkraftwiderstand, Laengsrichtung) statt vu_p (Platte, Querrichtung)
- mr_pb_p / mr_pb_n (Plattenbalken-Rissmoment) statt mr_p / mr_n (Platte-Rissmoment)
mu_max / mu_min entsprechen bei RibbedConcrete bereits dem Plattenbalken-Biegewiderstand.

Diagramme:
1. Biegewiderstand unten mu_max (MRd, Plattenbalken) vs. Bemessungsmoment positiv mEd_p
2. Biegewiderstand oben mu_min (MRd, Plattenbalken) vs. Bemessungsmoment negativ mEd_n
3. Querkraftwiderstand vu_PB_p (VRd, Plattenbalken) vs. Bemessungsquerkraft vEd
4. Rissmoment mr_pb_p (Plattenbalken) vs. mkd_p (positive Biegung)
5. Rissmoment mr_pb_n (Plattenbalken) vs. mkd_n (negative Biegung)
6. Durchbiegung w_install_adm (zulässig) vs. w_install_ger (gerissen)
7. Durchbiegung w_use_adm (zulässig) vs. w_use_ger (gerissen)
8. Durchbiegung w_app_adm (zulässig) vs. w_app_ger (gerissen)

Zusaetzlich wird eine kombinierte Uebersicht in relativen Werten erstellt (Ausnutzungsgrad je
Nachweis): Widerstand = 100 %, Ausnutzung = Einwirkung / Widerstand * 100 %.
...
ein Pfad als erstes Argument uebergeben werden - dieser hat dann Vorrang vor EXCEL_DATEI:
    py Member_data_analysis_rc_rib.py [<pfad_zur_excel_datei>] [sheet_name]
...
def lade_daten(excel_file, sheet_name):
    ...
    df["mu_min"] = df["mu_min"] / 1000
    df["mEd_n"] = df["mEd_n"] / 1000
    df["vu_PB_p"] = df["vu_PB_p"] / 1000
    df["vEd"] = df["vEd"] / 1000
    df["mr_pb_p"] = df["mr_pb_p"] / 1000
    df["mr_pb_n"] = df["mr_pb_n"] / 1000
    ...
    df["mEd_n_kleiner_als_mu_min"] = df["mEd_n"] < df["mu_min"]
    df["ved_groesser_als_vrd_pb"] = df["vEd"] > df["vu_PB_p"]
    ...
    df["mkd_p_groesser_als_mr_pb_p"] = df["mkd_p"] > df["mr_pb_p"]
    df["mkd_n_kleiner_als_mr_pb_n"] = df["mkd_n"] < df["mr_pb_n"]
...
def entferne_rahmen(ax):
    """Entfernt den oberen/rechten Rahmen, damit Daten auf der oberen Achsengrenze
    (z.B. eine Linie bei y = 100) nicht vom Rahmen ueberdeckt werden."""
...
DIAGRAMME = [
    {
        ...
        "labels": ["mu_max (MRd, Plattenbalken)", "mEd_p"],
        "ylabel": "Moment unten [kNm]",
        "bedingung": "med_groesser_als_mrd",
        "bedingung_label": "mEd_p > mu_max",
        "titel": "Biegewiderstand mu_max (MRd, Plattenbalken) vs. Bemessungsmoment mEd_p",
    },
    {
        "suffix": "mu_min_vs_mEd_n",
        "spalten": ["mu_min", "mEd_n"],
        "labels": ["mu_min (MRd, Plattenbalken)", "mEd_n"],
        "ylabel": "Moment oben [kNm]",
        "bedingung": "mEd_n_kleiner_als_mu_min",
        "bedingung_label": "mEd_n < mu_min",
        "titel": "Biegewiderstand mu_min (MRd, Plattenbalken) vs. Bemessungsmoment mEd_n",
    },
    {
        "suffix": "vEd_vs_vu_PB_p",
        "spalten": ["vu_PB_p", "vEd"],
        "labels": ["vu_PB_p (VRd, Plattenbalken)", "vEd"],
        "ylabel": "Querkraft [kN]",
        "bedingung": "ved_groesser_als_vrd_pb",
        "bedingung_label": "vEd > vu_PB_p",
        "titel": "Querkraftwiderstand vu_PB_p (VRd, Plattenbalken) vs. Bemessungsquerkraft vEd",
    },
    {
        "suffix": "mkd_p_vs_mr_pb_p",
        "spalten": ["mr_pb_p", "mkd_p"],
        "labels": ["mr_pb_p (Rissmoment, Plattenbalken)", "mkd_p"],
        "ylabel": "Rissmoment positiv [kNm]",
        "bedingung": "mkd_p_groesser_als_mr_pb_p",
        "bedingung_label": "mkd_p > mr_pb_p",
        "titel": "Rissmoment mr_pb_p (Plattenbalken) vs. Moment mkd_p (positive Biegung)",
    },
    {
        "suffix": "mkd_n_vs_mr_pb_n",
        "spalten": ["mr_pb_n", "mkd_n"],
        "labels": ["mr_pb_n (Rissmoment, Plattenbalken)", "mkd_n"],
        "ylabel": "Rissmoment negativ [kNm]",
        "bedingung": "mkd_n_kleiner_als_mr_pb_n",
        "bedingung_label": "mkd_n < mr_pb_n",
        "titel": "Rissmoment mr_pb_n (Plattenbalken) vs. Moment mkd_n (negative Biegung)",
    },
    {
        ...
        "ylabel": "Durchbiegung (Einbauten) w [m]",
    ...
    },
    {
        ...
        "ylabel": "Durchbiegung (Nutzung) w [m]",
    ...
    },
    {
        ...
        "ylabel": "Durchbiegung (Aussehen) w [m]",
...
# ==============================================================================
# PLOT: KOMBINIERTE UEBERSICHT (N ZEILEN x N SPALTEN)
# ==============================================================================
...
# ==============================================================================
# PLOT: KOMBINIERTE UEBERSICHT IN RELATIVEN WERTEN (WIDERSTAND = 100 %, AUSNUTZUNG IN %)
# ==============================================================================
def plot_kombiniert_relativ(df, spannweiten, dataset_name, output_prefix):
    fig, axes = plt.subplots(len(DIAGRAMME), len(spannweiten),
                              figsize=(2.6 * len(spannweiten), 3.6 * len(DIAGRAMME)), sharex="col", sharey=True)
    if len(spannweiten) == 1:
        axes = axes.reshape(-1, 1)

    for row_idx, diagramm_cfg in enumerate(DIAGRAMME):
        widerstand_spalte, einwirkung_spalte = diagramm_cfg["spalten"]
        widerstand_label, einwirkung_label = diagramm_cfg["labels"]
        for col_idx, l in enumerate(spannweiten):
            ax = axes[row_idx, col_idx]
            df_l = df[df["l_tot"] == l].sort_values("member_index")
            ausnutzung = df_l[einwirkung_spalte] / df_l[widerstand_spalte] * 100
            shade_condition_regions(ax, df_l["member_index"], df_l[diagramm_cfg["bedingung"]], FARBE_HINTERLEGUNG)
            ax.axhline(100, color=FARBE_1, linewidth=1.5, label=f"{widerstand_label} = 100 %")
            ax.plot(df_l["member_index"], ausnutzung, marker="x", markersize=1,
                    linewidth=1, color=FARBE_2, label=f"Ausnutzung {einwirkung_label}")
            ...
            if row_idx == len(DIAGRAMME) - 1:
                ax.set_xlabel("Member Nr.")
            if col_idx == 0:
                basis_label = diagramm_cfg["ylabel"].split(" [")[0]
                ax.set_ylabel(f"{basis_label} Ausnutzung [%]")
                ax.legend(loc="best", framealpha=0.9, fontsize=9)

    fig.legend(handles=[Patch(facecolor=FARBE_HINTERLEGUNG, alpha=0.6, label="Ausnutzung > 100 % (Widerstand überschritten)")],
               loc="lower center", bbox_to_anchor=(0.5, -0.01))

    fig.suptitle(f"Uebersicht: Ausnutzungsgrad (Einwirkung / Widerstand) je Nachweis und Member, "
                 f"kategorisiert nach Spannweite\n({dataset_name})", fontsize=16)
    fig.tight_layout(rect=[0, 0.02, 1, 0.96])

    for ext in ("png", "pdf"):
        pfad = f"{output_prefix}_combined_overview_relative.{ext}"
        fig.savefig(pfad, dpi=300 if ext == "png" else None, bbox_inches="tight")
    plt.close(fig)
    print(f"Diagramm gespeichert: {output_prefix}_combined_overview_relative.png / .pdf")
...
def main():
    ...
    plot_kombiniert_relativ(df, spannweiten, dataset_name, output_prefix)
