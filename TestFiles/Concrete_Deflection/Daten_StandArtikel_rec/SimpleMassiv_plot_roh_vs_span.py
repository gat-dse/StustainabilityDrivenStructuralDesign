...
in Abhaengigkeit der Spannweite l_tot dar - alle Beton-/Bewehrungs-Produktkombinationen
in EINEM Diagramm (eine Linie je Kombination, z.B. c44/r109).
...
df = pd.read_sql(f"SELECT Member_ID, l_tot, prod_id_concrete, prod_id_rebar, roh FROM {table_name}", con)
con.close()

for col in ["l_tot", "prod_id_concrete", "prod_id_rebar", "roh"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")
df = df.dropna(subset=["l_tot", "prod_id_concrete", "prod_id_rebar", "roh"])
...
for prod_c in concrete_prod_ids:
    ...
    for prod_r in rebar_prod_ids:
        ...
        ax.plot(df_sub["l_tot"], df_sub["roh"], marker="o", markersize=6, markeredgecolor="white",
                markeredgewidth=0.6, linewidth=2, color=farbe, linestyle=linien_stil[prod_r])

ax.set_xlabel("Spannweite l_tot [m]")
ax.set_ylabel("roh (unten) [-]")
ax.set_xticks(sorted(df["l_tot"].unique()))
ax.grid(True, linewidth=0.5, alpha=0.4)
ax.set_title("roh vs. Spannweite, je Beton- und Bewehrungs-Produkt\n"
...
fig.savefig("SimpleMassiv_roh_vs_span.png", dpi=300, bbox_inches="tight")
fig.savefig("SimpleMassiv_roh_vs_span.pdf", bbox_inches="tight")
print("Diagramm gespeichert: SimpleMassiv_roh_vs_span.png / .pdf")
