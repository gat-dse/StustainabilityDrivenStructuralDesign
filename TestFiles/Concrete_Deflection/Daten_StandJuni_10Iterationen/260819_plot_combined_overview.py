...
in einer Uebersichtsgrafik: 4 Zeilen (je eine Kenngroesse) x 5 Spalten (je eine Spannweite).
X-Achse je Spalte: Globaler Member-Index (1..60), jeder Member nur einmal.
...
# ==============================================================================
# PLOT: 4 ZEILEN (KENNGROESSEN) x 5 SPALTEN (SPANNWEITEN)
# ==============================================================================
fig, axes = plt.subplots(len(zeilen), len(spannweiten), figsize=(4 * len(spannweiten), 3.6 * len(zeilen)),
                          sharex="col")
...
for row_idx, zeile in enumerate(zeilen):
    for col_idx, l in enumerate(spannweiten):
        ...
        for col_name, color, label in zip(zeile["cols"], zeile["colors"], zeile["labels"]):
            ax.plot(df_l["member_index"], df_l[col_name], marker="o", markersize=5,
...
fig.suptitle("Uebersicht: Durchbiegung, fwger(phi=2), Bewehrungsgehalt und h/d je Member, "
             "kategorisiert nach Spannweite", fontsize=16)
fig.tight_layout(rect=[0, 0.02, 1, 0.98])

fig.savefig("260819_combined_overview.pdf", bbox_inches="tight")
fig.savefig("260819_combined_overview.png", dpi=300, bbox_inches="tight")
print("Diagramm gespeichert: 260819_combined_overview.pdf / .png")
