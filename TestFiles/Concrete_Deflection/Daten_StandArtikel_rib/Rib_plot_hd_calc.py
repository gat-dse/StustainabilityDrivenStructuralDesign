"""
Liest die Members_rc_rib_simple_massiv-Datenbank ein und stellt fuer jeden Member das
Verhaeltnis h/d_calc dar (berechnet aus den Rohzeilen h und d).
...
df = pd.read_sql(f'SELECT Member_ID, l_tot, "h/d_calc", w_use, w_use_ger FROM {table_name}', con)
con.close()

for col in ["l_tot", "h/d_calc", "w_use", "w_use_ger"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")
df = df.dropna(subset=["l_tot", "h/d_calc", "w_use", "w_use_ger"])
...
for ax, l in zip(axes, spannweiten):
    ...
    ax.plot(df_l["member_index"], df_l["h/d_calc"], marker="o", markersize=6,
            linewidth=2, color=farbe_hd, label="h/d_calc")
...
fig.suptitle("Verhaeltnis h/d_calc je Member, kategorisiert nach Spannweite\n"
             "(Members_rc_rib_simple_massiv)")
fig.tight_layout(rect=[0, 0.06, 1, 1])

fig.savefig("Rib_hd_calc.png", dpi=300, bbox_inches="tight")
fig.savefig("Rib_hd_calc.pdf", bbox_inches="tight")
print("Diagramm gespeichert: Rib_hd_calc.png / .pdf")
