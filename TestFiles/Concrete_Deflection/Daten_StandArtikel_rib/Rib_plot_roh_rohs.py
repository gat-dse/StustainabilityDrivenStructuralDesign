"""
Liest die Members_rc_rib_simple_massiv-Datenbank ein und stellt fuer jeden Member den
Bewehrungsgehalt roh (unten) und rohs (oben) dar.
...
df = pd.read_sql(f"SELECT Member_ID, l_tot, roh, rohs, w_use, w_use_ger FROM {table_name}", con)
con.close()

for col in ["l_tot", "roh", "rohs", "w_use", "w_use_ger"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")
df = df.dropna(subset=["l_tot", "roh", "rohs", "w_use", "w_use_ger"])
...
for ax, l in zip(axes, spannweiten):
    ...
    ax.plot(df_l["member_index"], df_l["roh"], marker="o", markersize=6,
            linewidth=2, color=farbe_roh, label="roh (unten)")
    ax.plot(df_l["member_index"], df_l["rohs"], marker="o", markersize=6,
            linewidth=2, color=farbe_rohs, label="rohs (oben)")
...
axes[0].set_ylabel("Bewehrungsgehalt [-]")
...
fig.suptitle("Bewehrungsgehalt roh vs. rohs je Member, kategorisiert nach Spannweite\n"
             "(Members_rc_rib_simple_massiv)")
fig.tight_layout(rect=[0, 0.06, 1, 1])

fig.savefig("Rib_roh_rohs.png", dpi=300, bbox_inches="tight")
fig.savefig("Rib_roh_rohs.pdf", bbox_inches="tight")
print("Diagramm gespeichert: Rib_roh_rohs.png / .pdf")
