"""
Liest die Members_rc_rib_simple_massiv-Datenbank ein und stellt fuer jeden Member die
Durchbiegung w_use (ungerissen) und w_use_ger (gerissen) dar.
...
df = pd.read_sql(f"SELECT Member_ID, l_tot, w_use, w_use_ger FROM {table_name}", con)
con.close()

for col in ["l_tot", "w_use", "w_use_ger"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")
df = df.dropna(subset=["l_tot", "w_use", "w_use_ger"])
...
# DESIGN
# ==============================================================================
farbe_w_use = "#0072B2"        # ungerissen
farbe_w_use_ger = "#D55E00"    # gerissen
...
for ax, l in zip(axes, spannweiten):
    ...
    ax.plot(df_l["member_index"], df_l["w_use"], marker="o", markersize=6,
            linewidth=2, color=farbe_w_use, label="w_use (ungerissen)")
    ax.plot(df_l["member_index"], df_l["w_use_ger"], marker="o", markersize=6,
            linewidth=2, color=farbe_w_use_ger, label="w_use_ger (gerissen)")
...
axes[0].set_ylabel("Durchbiegung w [m]")
...
fig.suptitle("Durchbiegung w_use vs. w_use_ger je Member, kategorisiert nach Spannweite\n"
             "(Members_rc_rib_simple_massiv)")
fig.tight_layout(rect=[0, 0.06, 1, 1])

fig.savefig("Rib_w_use_vs_w_use_ger.png", dpi=300, bbox_inches="tight")
fig.savefig("Rib_w_use_vs_w_use_ger.pdf", bbox_inches="tight")
print("Diagramm gespeichert: Rib_w_use_vs_w_use_ger.png / .pdf")
